...
- **前置條件（hard prerequisites）**：011a 開工前必須完成：
  ...
  3. **BE 020 補三個 admin GET endpoint**：`/cms/donation/charities`、`/cms/donation/charities/:id`、（之後）`/donation-projects`、`/sale-items` 對應；FE 端 `BackendAdminCharityDetail` Zod schema（[011 §5.4](./011-cms-resource-admin.md#54-be-硬依賴backendcharitydetail-缺欄位)）
