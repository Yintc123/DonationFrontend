...
- **狀態**：Draft（v0.3 — 與實作對齊：5 必填 + toast 點名、BFF POST 不 Zod 鎖 response、list 補 publish 欄位 + 已下架紅 row、session 顯式轉發、CSRF token 客戶端）
- **路徑（規劃）**：
...
| 0.3 | 2026-06-16 | **與實作對齊（impl retro-fit）**：(a) **必填擴增 2→5**：原本只 name / description，新增 `publishStartAt` / `publishEndAt` / `categoryIds (≥ 1)`，FormField 都掛紅星；(b) **`validate(form)` + toast 點名缺欄**：取代 `disabled={!isValid}` 反灰 button — 改成 button 永遠可按、點下去 `validate()` 回傳 `{ ok | missing: string[] }`、missing 非空 → `toast.error('請填寫：' + missing.join('、'))`；軟驗證（email / URL / 長度）也走同 list；(c) **BFF POST 不再用 `BackendAdminCharityDetail` Zod 鎖 response** — BE 020 §5.1.1 POST 回 public `CharityDetail`（fast-json-stringify strip 掉 5 個 admin lifecycle 欄）；舊版會在每次成功 create 後 502 ContractViolation。改 pass-through，因 FE submit 後 `router.replace` 回 list 不讀 body；(d) **List page 補欄**：「上架時間」「下架時間」用 local TZ 格式化（YYYY/MM/DD HH:MM；null → 「—」）；`isExpired(publishEndAt, now)` row → `text-brand` 紅字 + cell 內補「（已下架）」label；`now` 由 RSC 頂層抓一次共享；(e) **`<AdminTable>` 加 `rowClassName?: (row) => string\|undefined` prop**（[011d v0.2](./011d-cms-admin-ui-primitives.md)）支援 lifecycle 狀態著色；(f) **session 顯式轉發**：RSC `fetchAdminCharityList` / `fetchAdminCharityDetail` 與 BFF `/api/cms/charities/(POST/GET)` + `/api/cms/charities/[id]/(GET/PATCH)` 都 `session: await getSessionService().get()`（RSC）或 `session` from createAdminRoute handler args（BFF）。漏寫導致 backendFetch 不帶 Bearer → BE 401 → backendFetch 砍 session + 連環 bug；(g) **client-side CSRF**：`useCharityForm.handleSubmit` 在 POST/PATCH 前 `await getCsrfToken()`（[`src/lib/client/csrf.ts`](../../src/lib/client/csrf.ts)）→ `GET /api/csrf` → 拿 `session.csrfToken` 塞 `x-csrf-token` header（admin BFF 不 csrfExempt）；(h) **MultiSelectChips 選中 highlight 強化**（[011d v0.2](./011d-cms-admin-ui-primitives.md)）：bg-brand + text-white + font-medium + ring-2 ring-brand/40 ring-offset-1 + transition-colors |
