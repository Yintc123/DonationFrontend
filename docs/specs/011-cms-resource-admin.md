...
- **狀態**：Draft（v0.3 — 與實作對齊：`ensureAdminAccess` RSC 防護、JWT role decode、LoginCard 真實 password login、demo 註冊預設 ADMIN）
- **建立日期**：2026-06-16
- **依賴**：
...
| 路徑 | 動作 |
|---|---|
| [`/api/auth/login`](../../src/app/api/auth/login/route.ts)（v0.4 — 原 `/api/dev/login`） | 從真實 BE `/auth/login` 拿 access JWT → `resolveRole(me.role, accessToken)` decode role claim；BE `/auth/me` 目前不回 role 是預期（spec 008 §6.4），JWT claim 為唯一來源 |
...
| 0.3 | 2026-06-16 | **與實作對齊（impl 完成後 retro fit）**：(a) **`ensureAdminAccess<T>(fn)` RSC 防護**（[`src/lib/session/requireAdmin.ts`](../../src/lib/session/requireAdmin.ts)）— 包 RSC 內的 admin fetch，捕到 `UnauthenticatedError` (401) 或 `BackendClientError`(403) → `destroy session + redirect /?reason=cms-not-admin`，避免 token 過期 / admin 被降級時噴 generic error UI；(b) **`decodeJwtPayload` helper**（[`src/lib/auth/decodeJwtPayload.ts`](../../src/lib/auth/decodeJwtPayload.ts)）— BE `/auth/me` 不回 role（spec 008 §6.4；role 在 JWT claims 內），dev-login / future OAuth 需 decode access JWT 讀 role。§3.4 「需配套更新的登入路徑」表加註：me.role 預設要從 JWT 取；(c) **LoginCard 真實密碼登入**（[`src/app/LoginCard.tsx`](../../src/app/LoginCard.tsx)）— v0.2 之前 form 收 username/password 但 submit 不帶 body、`/api/dev/login` 全用 env 寫死；v0.3 form 送 `{ identifier, password }`、dev-login 接 body 用 caller credentials（無 body 時 fallback env）；(d) **`AuthRedirectToast` 加 `cms-not-admin` reason**（[`src/app/AuthRedirectToast.tsx`](../../src/app/AuthRedirectToast.tsx)）→ toast.error「需要管理員權限」；(e) **`getCsrfToken()` client helper**（[`src/lib/client/csrf.ts`](../../src/lib/client/csrf.ts)）— admin form 在 client 端 mutation 前 `GET /api/csrf` 拿 token，塞 `x-csrf-token` header；(f) **Demo policy：自助註冊（password / Google OAuth）一律設 `role: Role.ADMIN`**（[BE 008 §4.1 / BE 007 §10.4](../../../backend/src/lib/auth/service.ts)） — 後台 demo 直接可用；prod 應改 default USER + 顯式 promote flow；(g) BFF / RSC 呼 backendFetch admin endpoint 一律 `session: await getSessionService().get()` 才會自動帶 Bearer（v0.2 漏寫導致 401 砍 session 連環 bug） |
| 0.4 | 2026-06-17 | **`/api/dev/login` → `/api/auth/login` 改名**（隨 spec 005 v0.7、001g v0.2）：(a) §3.4 「需配套更新的登入路徑」表第一列從 `/api/dev/login` 改為 [`/api/auth/login`](../../src/app/api/auth/login/route.ts)；(b) 路徑搬移後 `ENABLE_DEV_LOGIN` env gate + `DEV_ADMIN_USERNAME / PASSWORD` body fallback 移除——dev-only 命名隨 v0.3 接真實 BE 已不再貼切；(c) v0.2 / v0.3 changelog 內的歷史 `/api/dev/login` 字串保留歷史描述不改；(d) 本 spec role 機制（decode JWT claim）邏輯不變。|
