...
- **狀態**：Draft（v0.13 — CTA 文字 `確認送出` → `確認捐款`，對齊「確認捐款資訊」hero 標題語意）
- **路徑（規劃）**：
...
<page.tsx> (RSC, async)
  ...
  └─ <DonationConfirmPage> ('use client')
        └─ <ConfirmPageShell title="確認捐款資訊" ctaLabel="確認捐款" isValid onSubmit>
...
整頁 `<form>` + 紅 hero + TopNav + sticky「確認捐款」按鈕由 [`<ConfirmPageShell>`](./009c-shared-confirm-ui.md#21-confirmpageshell--整頁外殼) 統一接管：
...
| 0.13 | 2026-06-16 | **CTA 文字 `確認送出` → `確認捐款`**：對齊本頁 hero 標題「確認捐款資訊」與整個 donation flow 的「捐款」語意（v0.1 沿用既有 detail 紅 pill「確認送出」屬 generic 推斷，缺乏針對性）。改動 §3 元件樹、§6 sticky 句子、§6.1 JSX 範例三處 `ctaLabel` literal；component test 4b / 5 的 sticky button name 斷言同步替換。Purchase 流程（009b）維持「確認送出」不變（義賣商品購買語意更接近 generic 送出）。差異化載於 [009 §3.4](./009-checkout-confirm.md#34-sticky-bottom-cta) 與 [009c §2.1/§2.6](./009c-shared-confirm-ui.md#21-confirmpageshell--整頁外殼) prop 註解 [註:purchase 隨後於 [009b v0.11](./009b-purchase-confirm.md) 改為「確認購買」,見 [009 v0.10](./009-checkout-confirm.md)] |
