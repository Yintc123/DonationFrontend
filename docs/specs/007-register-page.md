...
- **路徑（backend）**：
  - `POST /v1/auth/register` — **已於 [backend spec 008 v0.6 §4 / §8.1](../../../backend/docs/specs/008-auth-flow-password.md)** 規範並由 BE 實作
  - `GET /v1/auth/me` — 註冊後拿 user profile（[BE 008 §6.4](../../../backend/docs/specs/008-auth-flow-password.md)；register response 本身不含 user）
- **依賴**：
...
```
/ (LoginCard) ──「建立帳號」──▶ /register ──「註冊」──▶ /dashboard（auto-login）
                                  │
                                  └──「我已有帳號」──▶ /
```

註冊成功 = backend 建 Account + PasswordCredential → BFF 拿到 tokens 後再打 `GET /v1/auth/me` 取 user → 建 iron-session（與 [dev/login](../../src/app/api/dev/login/route.ts) 同套）→ 帶 Set-Cookie 回 client → client `router.push('/dashboard')`。**不需要使用者再回首頁登入一次**。
...
| 註冊成功（200）| `router.push('/dashboard')`；不顯示成功 toast（auto-redirect 已是回饋）|
...
> **為何要兩段**：BE 008 §8.1 的 register response 只有 tokens、不含 user 物件（避免 endpoint 同時做兩件事的職責汙染）。FE iron-session 需要 user 才能在 dashboard 顯示「歡迎 alice」。GET /auth/me 是 BE 提供的官方取 user profile 入口（[BE 008 §6.4](../../../backend/docs/specs/008-auth-flow-password.md)）。
...
| 假設 | BE 出處 | FE 動作 |
|---|---|---|
| `POST /v1/auth/register` body shape: `{ username?, email?, password }` | [BE 008 §4.1 / §8.1](../../../backend/docs/specs/008-auth-flow-password.md) | FE 只送 username + password |
...
| `GET /v1/auth/me` 回 `{ id, username, email, displayOrder, role, createdAt, updatedAt, lastLoginAt, lastLoginType }` | [BE 008 §6.4](../../../backend/docs/specs/008-auth-flow-password.md) | BFF 把 user 物件包進 iron-session |
...
`/ → /register → 填表 → submit → /dashboard` 串完整 flow。BE 008 已實作 register endpoint，本地用 USE_MOCK=1 仍可以加 mock dispatcher 跑通；後續另起。
