...
- **首頁 + 後台 placeholder（v0.9）** — `/` 顯示登入卡（帳號 / 密碼 + 「登入後台」+「建立帳號」）+ skip link 「demo」→ `/donation`；「登入後台」POST `/api/auth/login`（v0.14 — 原 `/api/dev/login`；BFF 橋接 BE `/auth/login` + `/auth/me`，需真實帳密）→ `/cms`（v0.13 — 原 `/dashboard`，改名對齊 charity / project / sale-item CRUD 業務語意）；「建立帳號」→ `/register`（v0.12 — 原 `/admin`，因 BE `role=ADMIN` 命名衝突而搬家；spec 007 v0.2 已實作真實註冊 + auto-login 跳 `/cms`）；`/cms` 為 placeholder（`/admin` 留給未來真正 admin 後台），後台實際 CRUD 功能未開發
