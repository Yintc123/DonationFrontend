...
- **首頁 + 後台 placeholder（v0.9）** — `/` 顯示登入卡（帳號 / 密碼 + 「登入後台」+「建立帳號」）+ skip link 「demo」→ `/donation`；「登入後台」POST `/api/dev/login`（dev only，無需真實帳密驗證）→ `/dashboard`；「建立帳號」→ `/register`（v0.12 — 原 `/admin`，因 BE `role=ADMIN` 命名衝突而搬家）；`/dashboard`、`/register` 為 placeholder（`/admin` 留給未來真正 admin 後台），後台實際功能未開發
